...
async def handle_maker(body: bytes, request_host: str = None) -> dict:
    ...
    update_channel = info_namero.get("info", {}).get("updatechannel", "Namerobots")
    ...
    if not start_msg:
        start_msg = "• مرحبا بك عزيزي Namero في مصنع البوتات الخدمي 📬\n\n- اصنع العديد من البوتات بكل سهوله من الاسفل 🔥"
    if not info_kl:
    ...
    if data == "botsmember":
        ...
        if str(from_id) in botfree2 and botf2.strip():
                ...
                if len(idbot_b) > 4:
                    ...
                    if file_exists(info_path):
                        ...
                        keyboard["inline_keyboard"].append([
                            {"text": userbot_b, "url": f"t.me/{userbott}"},
                            {"text": s_p_p1_b, "url": f"t.me/{userbott}"},
                            {"text": "معلومات اكثر ", "callback_data": in_val}
                        ])
            ...
            await bot_call(token, "editMessageText", {"chat_id": chat_id, "message_id": message_id, "text": " هذه هي قائمة بوتاتك المصنوعة \n\n", "reply_markup": json.dumps(keyboard)})
        else:
            await bot_call(token, "editMessageText", {"chat_id": chat_id, "message_id": message_id, "text": "• لايوجد لديك اي بوت حاليا ❌", "reply_markup": json.dumps({"inline_keyboard": [[{"text": "• رجوع •", "callback_data": "freebot"}]]})})
        return {"ok": True}
    ...
    if data and data.startswith("infobot "):
        ...
        if file_exists(info_path):
            ...
            co_d = len(mm_d)
            await bot_call(token, "editMessageText", {"chat_id": chat_id, "message_id": message_id, "text": f" معلومات البوت المصنوع 📝: \n----------------------------\n- الاسم: `{name}`\n• النوع : {s_p_p1_d}\n• الايدي : `{idbots_d}`\n• الاسم : `{namebot_d}`\n- التوكن : محمي\n \n• عدد الاعضاء المشتركين في البوت : {co_d}\n", "reply_to_message_id": message_id, "parse_mode": "MarkDown", "disable_web_page_preview": "true", "reply_markup": json.dumps({"inline_keyboard": [
                [{"text": " حذف البوت", "callback_data": "deletebot " + userbot_d}, {"text": "نقل الملكية", "callback_data": "naglbotmember " + userbot_d}],
...
# ══════════════════════════════════════════════════════════════════════════════
# نظام Polling — يعمل عند تشغيل الملف مباشرةً: python maker_handler.py
# ══════════════════════════════════════════════════════════════════════════════
        ...
        try:
            ...
            for update in updates:
                ...
                try:
                    ...
                    print(f"[Maker] ❌ خطأ في التحديث {update.get('update_id')}: {e}")
            ...
            if error_count >= _MAX_ERRORS:
                await asyncio.sleep(_RETRY_DELAY * 2)
                error_count = 0
            else:
