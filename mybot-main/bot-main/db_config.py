...
def db_read(file_path: str, default: str = "") -> str:
    ...
    try:
        with _lock:
            ...
            try:
                row = conn.execute(
                    "SELECT content_data FROM content_storage WHERE content_key = ? AND content_type = 'text'",
                    (file_path,)
                ).fetchone()
...
def db_write(file_path: str, content: str) -> bool:
    ...
    try:
        with _lock:
            ...
            try:
                conn.execute(
                    "INSERT OR REPLACE INTO content_storage (content_key, content_type, content_data, updated_at) "
...
def db_delete(file_path: str) -> bool:
    ...
    try:
        with _lock:
            conn = _get_conn()
            try:
                conn.execute("DELETE FROM content_storage WHERE content_key = ?", (file_path,))
                conn.commit()
                return True
            finally:
                conn.close()
...
def db_read_json(file_path: str, default: Optional[Dict] = None) -> Dict:
    ...
    try:
        with _lock:
            ...
            try:
                row = conn.execute(
                    "SELECT content_data FROM content_storage WHERE content_key = ? AND content_type = 'json'",
                    (file_path,)
                ).fetchone()
