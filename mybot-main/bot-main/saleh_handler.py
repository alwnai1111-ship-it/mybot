...
def _get_setting(bot_dir: str) -> dict:
    ...
    def fetch_setting():
        ...
        return setting
    return _get_cached(f"setting_{bot_dir}", fetch_setting)
...
async def handle_namero4(
    ...
    # ══════════════════════════════════════════════════════════════════════
    if S_P_P1 and not S_P_P1.startswith("/") and NaMerOset.get("azrar", {}).get(S_P_P1, {}).get("text"):
    ...
    if (
        ...
        if str(from_id) not in ban:
            ...
            # إرسال الرسالة الثانية (محتوى الرسالة + الأزرار)
    ...
    async def _fwd_media(content_flag: str) -> None:
        if content_flag in ("❌", "") or not content_flag:
            ...
            # إذا نجح forward، أرسل الأزرار مع نص مختصر (رسالة ثانية)
            if get_r.get("ok"):
